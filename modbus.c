...
extern volatile uint8_t response[125]; //Enough to return all holding-r's
extern volatile uint8_t received[125]; //Enough to write all holding-r's
...
void readReg(void)
{
    ...
    writeEnable = 1;
    for (i = 0; i != j; i++) {
        while (U1STAbits.TRMT == 0); //Change this to Busy1USART for double USART PIC's
        U1TXREG = response[i];
    }
    writeEnable = 0;
    j=0;
